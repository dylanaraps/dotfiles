...
#define GetY(x,y)   ( textureLod(LOWRES_UV_raw, ddxddy*(pos+vec2(x,y)+0.5), 0.0).a )
//Downsampled result
#define Diff(x,y)   ( textureLod(LOWRES_UV_raw, ddxddy*(pos+vec2(x,y)+0.5), 0.0) )
...
vec4 hook() {    
    ...
    for (int Y = minX; Y <= maxX; Y++)
    {
        float dI2 = sqr(acuity*(c0.x - GetY(X,Y)));
        //float dXY2 = sqr((vec2(X,Y) - offset)/radius);
        ...
        vec4 d = Diff(X,Y);
        d.yz -= textureLod(CHROMA_raw, ddxddy*(pos+vec2(X,Y)+0.5), 0.0).xy;
        diff += weight*d;
        weightSum += weight;
    }
    diff /= weightSum;
